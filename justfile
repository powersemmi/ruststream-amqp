...
check:
    ...
    # Its own leg follows.
    cargo clippy --workspace --exclude ruststream-amqp-bench --all-targets --all-features -- -D warnings
    cargo clippy -p ruststream-amqp-bench --all-targets -- -D warnings
    cargo check --workspace --exclude ruststream-amqp-bench --all-targets --all-features
    cargo check -p ruststream-amqp-bench --all-targets
...
# What this crate costs over the fe2o3-amqp client it wraps: two scenarios run as a RustStream
...
bench *ARGS: brokers-up
    ...
    RUSTSTREAM_BENCH_OUT="$PWD/target/bench-paired.json" \
        cargo bench -p ruststream-amqp-bench --bench paired {{ ARGS }}
...
bench-code *ARGS:
    mkdir -p target
    RUSTFLAGS="" cargo bench -p ruststream-amqp --features testing \
