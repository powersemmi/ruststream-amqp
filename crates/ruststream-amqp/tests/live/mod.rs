//! The gate the live suites share: when a run is supposed to reach a broker, a skip is a defect.
//!
//! A live test skips when `AMQP_TEST_URL` is unset, which is what keeps the suites usable during
//! development: `cargo test` on a laptop with no stand passes. The same skip in CI is a lie,
//! because the job started a broker first, and a suite that returns before its first assertion
//! reports `ok` exactly like one that ran. The live job therefore sets `RUSTSTREAM_REQUIRE_LIVE`,
//! and under that flag a skip becomes a failure naming what it wanted.
...
/// The broker URL from `name`, or `None` to skip the test.
///
/// # Panics
///
...
pub(crate) fn url(name: &str) -> Option<String> {
    match std::env::var(name) {
        ...
        _ => {
            ...
            eprintln!("{name} is not set; skipping the live-broker suite");
            None
        }
    }
}
