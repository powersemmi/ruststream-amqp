...
impl Bus {
    ...
    pub(crate) fn subscribe(
        ...
        self.router.subscribe(address, routing)
    }
    ...
    /// Routes one framed message to the subscriptions on `address`.
    pub(crate) fn route(&self, address: &str, delivery: &Delivery) {
        self.router.publish(address, delivery, self.coordinator());
    }
