...
//! on [`AmqpTestBroker`], this crate's in-process transport. The transport resolves the crate's
//! own subscription descriptor, [`AmqpAddress`](ruststream_amqp::AmqpAddress), with its
//! validation, its terminus routing and its settle mode. It pairs the crate's production publish
//! policy, [`AmqpPublish`](ruststream_amqp::AmqpPublish), into its publisher. Its subscription
//! yields through the same client-side buffer the production subscriber batches with. The message
//! handle and the publisher are its own in-process counterparts, so the conversion between an
//! `fe2o3-amqp` message and the crate's (application properties into headers, the dispositions
//! behind `ack`, the assembly of a transfer) is not in the number. What comes out is what a
//! message costs in the crate's routing and settlement and the framework's dispatch above them,
//! with no socket and no server in it. The comparison against the raw `fe2o3-amqp` client, over a
//! real broker, is the other table on the benchmarks page, and that conversion is in it.
//!
//! A publish on the in-process transport allocates where the production publisher does not. The
//! router keeps a record of every message for a test to assert on and a turn among competing
//! consumers, which costs three strings per publish, and the publisher freezes the body it is
//! handed. Four of the reply scenario's five allocations per message are these; the fifth is the
//! encoded body, which the production publisher hands to the client as it is.
//!
//! The in-process transport is behind the `testing` feature, which compiles the framework's test
//! hooks in as well. Outside a `TestApp` run they are installed empty on a single delivery. The
//! batch path copies every payload of a batch for the harness's record whether a harness runs or
//! not, and those copies are the batch scenario's two allocations per message.
//!
...
/// The address every scenario delivers on. A handler names it in its own `#[subscriber(..)]`
/// descriptor, `AmqpAddress::queue("orders")`.
...
pub type Mount<'a> = &'a mut BrokerScope<AmqpTestBroker, Identity, (), Latch>;
...
fn fill(broker: &AmqpTestBroker, runtime: &Runtime, count: usize) {
