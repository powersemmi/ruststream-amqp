...
- **Delayed retries have somewhere to land.** The protocol has no delayed redelivery, so `retry_after` is the framework's deferred re-publish, and every subscription here reports the address a publisher reaches it at again. A scope wired with `retry_via` therefore starts whichever way the subscription was named.
