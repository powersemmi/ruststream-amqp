...
- **Pages.** A handler taking a slice gets pages of the size its mount site names (`.batch(nonzero!(32))`). A transfer carries one message, so the pages are assembled on the client and `page_wait` caps how long a partial one waits - the mount site reads the same as on a broker that pages on the wire.
