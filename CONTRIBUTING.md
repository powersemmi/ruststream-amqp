# Contributing to ruststream-amqp

`ruststream-amqp` is the AMQP 1.0 broker crate of RustStream. The framework's core crate lives in
...
git clone https://github.com/powersemmi/ruststream-amqp.git
```
...
- **Docker** with Compose, for the ActiveMQ Artemis stand the live suite and the benchmarks run against.
- Per task:
...
just test-brokers   # the live suite against ActiveMQ Artemis
...
starts the Compose stand, runs the whole suite against Artemis with the live tests required, and
stops the stand.
...
From the core, `just brokers amqp` runs the same command against the core's working tree.
...
```bash
just brokers-up
AMQP_TEST_URL=amqp://artemis:artemis@127.0.0.1:5672 RUSTSTREAM_REQUIRE_LIVE=1 \
