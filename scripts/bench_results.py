...
FEATURES = "ruststream-amqp default (none), ruststream macros,json"
...
def main() -> int:
    ...
    if code:
        ...
        machine = environment()
        # The round trip is a measurement, not a machine fact, but it is published with the
        # machine because it is what the `broker_bound` arithmetic is done with and a reader
        # checking that arithmetic needs it beside the numbers.
        machine["round_trip"] = (
            f"{summary['round_trip_micros']} us"
            f" (median of {summary['round_trip_samples']} sends waiting for their disposition)"
        )
        document = {
            "schema": 3,
            "crate": "ruststream-amqp",
